export const ORGANIZATIONS = ['PoliceMSP', 'RABMSP', 'BGBMSP', 'CustomsMSP'] as const;
...
export const PURPOSE_CODES = ['ACTIVE_INVESTIGATION', 'BORDER_SCREENING', 'CUSTOMS_INQUIRY'] as const;
export const DISCLOSURE_SCOPES = ['IDENTITY_CONFIRMATION', 'CASE_REFERENCE', 'CONTACT_CHANNEL', 'ELIGIBILITY_CATEGORY'] as const;
...
export type LedgerRecord = QueryRequest | MatchAttestation | AccessRequest | AuthorizationDecision | DisclosureReceipt;

export interface SecurityEvent {
  eventId: string;
  actorId?: string;
  actorOrg?: Organization;
  type: 'MISSING_CASE' | 'INACTIVE_CASE' | 'BUDGET_EXCEEDED' | 'INVALID_SCOPE' | 'REVOKED_USER' | 'UNAUTHORIZED_ACTION';
