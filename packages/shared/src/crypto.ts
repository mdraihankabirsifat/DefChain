...
export function canonicalizeSyntheticIdentifier(value: string): string {
  ...
  if (!/^TEST-NID-[0-9]{4}$/.test(canonical))
    throw new Error("INVALID_SYNTHETIC_IDENTIFIER");
  return canonical;
}

export function protectedMatchToken(
  identifier: string,
  key: string,
  epoch: string,
): string {
  ...
  return createHmac("sha256", key)
    .update(`defchain:identifier:${epoch}:${canonical}`, "utf8")
    .digest("hex");
}
...
export function signReceipt(
  ...
  return sign(null, Buffer.from(payloadHash, "hex"), privateKeyPem).toString(
    "base64",
  );
}
