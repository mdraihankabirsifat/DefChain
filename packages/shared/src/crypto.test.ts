...
describe("privacy helpers", () => {
  it("canonicalizes before deterministic protected matching", () => {
    expect(protectedMatchToken(" test-nid-0001 ", "key", "epoch")).toBe(
      protectedMatchToken("TEST-NID-0001", "key", "epoch"),
    );
  });
  it("encrypts with fresh GCM nonces and decrypts", () => {
    ...
    expect(decryptPayload(a, "secret")).toEqual({
      caseReference: "SYNTHETIC-RAB-01",
    });
  });
