...
export async function callAdapter<T>(
  ...
  const signature = createHmac("sha256", key)
    ...
    .digest("hex");
  const response = await fetch(`${urls[org]}${route}`, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      "x-defchain-timestamp": timestamp,
      "x-defchain-nonce": nonce,
      "x-defchain-signature": signature,
    },
    body,
  ...
  if (!response.ok)
    throw new Error(
      `ADAPTER_REJECTED: ${result.error?.code ?? response.status}`,
    );
  return result;
}
