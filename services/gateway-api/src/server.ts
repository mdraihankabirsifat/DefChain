...
function event(
  ...
  context: SecurityEvent["safeContext"],
): void {
  database.addSecurityEvent({
    eventId: safeRandomId("security"),
    actorId: user?.id,
    actorOrg: user?.organization,
    type,
    safeContext: context,
...
function requireAuth(
  ...
  try {
    ...
    if (!user?.active) {
      event(user, "REVOKED_USER", { route: req.path });
      throw new DefChainError("USER_REVOKED", "User disabled or revoked", 403);
    }
    req.user = user;
    next();
  } catch (error) {
    next(
      error instanceof DefChainError
        ? error
        : new DefChainError(
            "INVALID_SESSION",
...
function role(...roles: AppUser["role"][]) {
  return (req: AuthRequest, _res: Response, next: NextFunction) => {
    if (!req.user || !roles.includes(req.user.role)) {
      event(req.user, "UNAUTHORIZED_ACTION", { route: req.path });
      next(new DefChainError("FORBIDDEN", "Role is not permitted", 403));
      return;
    }
    next();
  };
...
app.use(
  pinoHttp({
    redact: [
      ...
      "req.body.password",
      "req.body.syntheticIdentifier",
      "req.body.protectedToken",
...
app.post("/api/v1/auth/login", async (req, res, next) => {
  try {
    ...
    if (!user.active) {
      event(user, "REVOKED_USER", { route: "/auth/login" });
      throw new DefChainError("USER_REVOKED", "User disabled or revoked", 403);
    }
    res.json({
      token: jwt.sign({ sub: user.id }, jwtSecret, {
        expiresIn: "2h",
        issuer: "defchain-demo",
      }),
      user: safeUser(user),
    });
  } catch (error) {
...
app.post(
  ...
  async (req: AuthRequest, res, next) => {
    try {
      ...
      const demoCase = database.caseById(input.caseId);
      if (!demoCase) {
        event(appUser, "MISSING_CASE", {
          caseReferenceHash: sha256(input.caseId),
        });
        throw new DefChainError("CASE_NOT_FOUND", "Case not found", 404);
      }
      if (demoCase.status !== "ACTIVE") {
        event(appUser, "INACTIVE_CASE", {
          caseReferenceHash: sha256(input.caseId),
          status: demoCase.status,
        });
        throw new DefChainError("CASE_INACTIVE", "Case must be active", 409);
      }
      if (
        demoCase.owner_org !== appUser.organization ||
        demoCase.allowed_purpose !== input.purposeCode
      )
        throw new DefChainError(
          "PURPOSE_NOT_ALLOWED",
          "Purpose is not authorized for this case",
          403,
        );
      if (appUser.query_count >= appUser.query_budget) {
        event(appUser, "BUDGET_EXCEEDED", { budget: appUser.query_budget });
        throw new DefChainError(
      ...
      const query = await fabric.submit(
        ...
        JSON.stringify({
          queryId,
          requesterOrg: appUser.organization,
          opaqueCaseRef: demoCase.opaque_ref,
...
app.get(
  "/api/v1/workflows/:queryId",
  requireAuth,
  async (req: AuthRequest, res, next) => {
    try {
      res.json({
        records: await fabric.evaluate<LedgerRecord[]>(
  ...
  async (req: AuthRequest, res, next) => {
    try {
      ...
      if (
        ...
        event(req.user!, "INVALID_SCOPE", {
          requestedScopeCount: suppliedScopes.length,
        });
        throw new DefChainError(
          "INVALID_SCOPE",
          "One or more scopes are not allowed",
          400,
        );
      }
      ...
      if (!query || query.recordType !== "QueryRequest")
        throw new DefChainError("QUERY_NOT_FOUND", "Query not found", 404);
      const access = await fabric.submit<AccessRequest>(
        req.user!.organization,
        "CreateAccessRequest",
        JSON.stringify({
          requestId,
          queryId: input.queryId,
          requesterOrg: req.user!.organization,
          providerOrg: input.providerOrg,
          requestedScopes: input.requestedScopes,
          purposeCode: query.purposeCode,
          justificationHash: sha256(input.justification),
        }),
      );
  ...
  async (req: AuthRequest, res, next) => {
    try {
      ...
      const records = (
        await Promise.all(
          queries
            .filter((r) => r.recordType === "QueryRequest")
            .map((r) =>
              fabric.evaluate<LedgerRecord[]>(
                req.user!.organization,
                "GetWorkflow",
                (r as { queryId: string }).queryId,
              ),
            ),
        )
      ).flat();
      const decided = new Set(
        records
          .filter((record) => record.recordType === "AuthorizationDecision")
          .map((record) => (record as { requestId: string }).requestId),
      );
      res.json({
        requests: records.filter(
          (record) =>
            record.recordType === "AccessRequest" &&
            record.providerOrg === req.user!.organization &&
            !decided.has((record as { requestId: string }).requestId),
        ),
      });
    } catch (error) {
      next(error);
...
app.post(
  "/api/v1/provider/requests/:requestId/decision",
  requireAuth,
  role("PROVIDER_OFFICER"),
  async (req: AuthRequest, res, next) => {
    try {
...
app.post(
  ...
  async (req: AuthRequest, res, next) => {
    try {
      ...
      if (access.requesterOrg !== req.user!.organization)
        throw new DefChainError(
          "FORBIDDEN",
...
app.get("/api/v1/security-events", requireAuth, role("AUDITOR"), (_req, res) =>
  res.json({ events: database.securityEvents() }),
);
app.post(
  "/api/v1/demo/reset",
  requireAuth,
  ...
  (_req, res, next) => {
    if (process.env.DEMO_MODE !== "true") {
      next(new DefChainError("DEMO_MODE_DISABLED", "Reset disabled", 403));
      return;
    }
    database.resetDemo();
    res.json({
      reset: "local demo counters and security events",
      ledgerResetCommand: "npm run reset",
    });
  },
);
...
app.use((error: unknown, req: Request, res: Response, _next: NextFunction) => {
  ...
  if (error instanceof DefChainError) {
    res
      .status(error.status)
  ...
  res.status(isFabric ? 503 : isValidation ? 400 : 500).json({
    error: {
      code: isFabric
        ...
        : isValidation
          ...
          : "INTERNAL_ERROR",
      message: isFabric
        ? "Blockchain unavailable; ledger-changing actions are blocked."
        : isValidation
...
if (
  ...
  app.listen(port, bindHost, () =>
    console.log(`DefChain gateway API listening on http://localhost:${port}`),
  );
}
