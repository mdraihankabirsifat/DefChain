...
describe.skipIf(!enabled)("real Fabric integration", () => {
  ...
  it("enforces MSP authority and immutable duplicate keys on the real network", async () => {
    ...
    await expect(fabric.submit("PoliceMSP", "CreateQueryRequest", JSON.stringify(input))).rejects.toThrow("ERR_DUPLICATE_ID");
    await expect(fabric.submit("PoliceMSP", "CreateMatchAttestation", JSON.stringify({ attestationId: safeRandomId("attest"), queryId, providerOrg: "RABMSP", result: "MATCH", policyVersion: "demo-1" }))).rejects.toThrow("ERR_WRONG_MSP");
  });
});
