...
describe.skipIf(!enabled)("real Fabric integration", () => {
  ...
  it("commits and queries the complete five-record workflow", async () => {
    const queryId = safeRandomId("query");
    const requestId = safeRandomId("request");
    const query = await fabric.submit<{ txId: string }>(
      "PoliceMSP",
      "CreateQueryRequest",
      JSON.stringify({
        queryId,
        requesterOrg: "PoliceMSP",
        opaqueCaseRef: "integration_opaque_case_ref",
    ...
    const match = await fabric.submit<{ txId: string }>(
      "RABMSP",
    ...
    const access = await fabric.submit<{ txId: string }>(
      "PoliceMSP",
      "CreateAccessRequest",
      JSON.stringify({
        requestId,
        ...
        purposeCode: "ACTIVE_INVESTIGATION",
        justificationHash: "a".repeat(64),
      }),
    );
    const decision = await fabric.submit<{ txId: string }>(
      "RABMSP",
      "RecordAuthorizationDecision",
      JSON.stringify({
        decisionId: safeRandomId("decision"),
        requestId,
        providerOrg: "RABMSP",
        decision: "PARTIAL",
        approvedScopes: ["IDENTITY_CONFIRMATION"],
        reasonCode: "INTEGRATION_POLICY",
        expiresAt: new Date(Date.now() + 600_000).toISOString(),
        policyVersion: "demo-1",
      }),
    );
    const receipt = await fabric.submit<{ txId: string }>(
      "RABMSP",
      ...
      JSON.stringify({
        ...
        providerSignature: "c".repeat(64),
        signatureAlgorithm: "Ed25519",
      }),
    );
    const records = await fabric.evaluate<
      Array<{ recordType: string; txId: string }>
    >("PoliceMSP", "GetWorkflow", queryId);
  ...
  it("enforces MSP authority and immutable duplicate keys on the real network", async () => {
    ...
    const input = {
      queryId,
      requesterOrg: "PoliceMSP",
      opaqueCaseRef: "integration_negative_case_ref",
      purposeCode: "ACTIVE_INVESTIGATION",
      targetOrganizations: ["RABMSP"],
      policyVersion: "demo-1",
      createdByRole: "INVESTIGATOR",
    ...
    await expect(
      fabric.submit("PoliceMSP", "CreateQueryRequest", JSON.stringify(input)),
    ).rejects.toThrow("BLOCKCHAIN_UNAVAILABLE_OR_REJECTED");
    await expect(
      fabric.submit(
        "PoliceMSP",
        "CreateMatchAttestation",
    ...
    await expect(
      fabric.evaluate("PoliceMSP", "GetRecord", `MATCH::${queryId}::RABMSP`),
    ).rejects.toThrow();
  }, 120_000);
});
